...
// is the command's alone, and the wait ends when the process does rather than
// riding out the timeout in poll slices.
@(test)
test_run_timeout_keeps_a_fast_command :: proc(t: ^testing.T) {
    ...
    elapsed := time.tick_since(start)
    testing.expectf(t, elapsed < 5 * time.Second, "the call took %v against a 20s timeout", elapsed)
