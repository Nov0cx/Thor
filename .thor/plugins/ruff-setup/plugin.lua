-- Detects whether ruff answers on PATH and, if not, adds a workspace task
...
-- directly. Mirrors plugins/basedpyright-setup/plugin.lua; this workspace's
-- .thor/lsp.json runs ruff (not basedpyright) as the .py/.pyi server, since
-- ruff answers textDocument/formatting and basedpyright never does.

local TASKS_FILE = ".thor/tasks.json"

local SERVER = {
    id = "ruff", label = "ruff (Python)", bin = "ruff",
    install = {
        windows = "pip install -U ruff",
        any     = "pip3 install -U ruff",
    },
    hint = "Installs the ruff binary beside your other Python scripts — make sure that folder is on PATH. pipx install ruff also works and keeps it isolated.",
}
...
thor.on_command("ruff-status", function()
    local path = which(SERVER.bin)
    local out = "# " .. SERVER.label .. " Setup\n\nChecked against: `" .. OS_NAME .. "`\n\n" ..
        (path ~= "" and ("found: `" .. path .. "`\n") or "not found on PATH\n")
    thor.doc(".thor/ruff-status.md", out, true)
end)

thor.on_command("ruff-install", function()
    if which(SERVER.bin) ~= "" then
        thor.print("\n[ruff-setup] " .. SERVER.label .. " is already on PATH\n")
        return
    end

    local command = install_command()
    if not command then
        thor.print("\n[ruff-setup] " .. SERVER.label .. ": no automatic installer for " .. OS_NAME .. "\n" ..
    ...
    thor.confirm("Install " .. SERVER.label .. " via: " .. command .. "?", function()
        ...
        if not ok then
            thor.print("\n[ruff-setup] could not add a task (" .. err .. "); run this yourself instead:\n> " .. command .. "\n")
            return
        end
        thor.print("\n[ruff-setup] added task \"" .. task_name .. "\" — run it from the titlebar Tasks selector.\n" ..
...
thor.menu("LSP Setup", {
    {label = "ruff: Check Status",     command = "ruff-status"},
    {label = "ruff: Install Missing…", command = "ruff-install"},
})
