-- Detects whether lua-language-server answers on PATH and, if not, adds a
...
thor.on_command("lua-language-server-install", function()
    if which(SERVER.bin) ~= "" then
        thor.print("\n[lua-language-server-setup] " .. SERVER.label .. " is already on PATH\n")
        return
    end
    ...
    thor.confirm("Install " .. SERVER.label .. " via: " .. command .. "?", function()
        ...
        if not ok then
            thor.print("\n[lua-language-server-setup] could not add a task (" .. err .. "); run this yourself instead:\n> " .. command .. "\n")
            return
        end
        thor.print("\n[lua-language-server-setup] added task \"" .. task_name .. "\" — run it from the titlebar Tasks selector.\n" ..
...
thor.menu("LSP Setup", {
    {label = "lua-language-server: Check Status",     command = "lua-language-server-status"},
    {label = "lua-language-server: Install Missing…", command = "lua-language-server-install"},
})
