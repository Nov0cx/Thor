-- Detects whether basedpyright-langserver answers on PATH and, if not, adds
-- a workspace task that installs it. Installers commonly run well past the
...
thor.on_command("basedpyright-status", function()
    local path = which(SERVER.bin)
    local out = "# " .. SERVER.label .. " Setup\n\nChecked against: `" .. OS_NAME .. "`\n\n" ..
        (path ~= "" and ("found: `" .. path .. "`\n") or "not found on PATH\n")
    thor.doc(".thor/basedpyright-status.md", out, true)
end)

thor.on_command("basedpyright-install", function()
    if which(SERVER.bin) ~= "" then
        thor.print("\n[basedpyright-setup] " .. SERVER.label .. " is already on PATH\n")
        return
    end

    local command = install_command()
    if not command then
        thor.print("\n[basedpyright-setup] " .. SERVER.label .. ": no automatic installer for " .. OS_NAME .. "\n" ..
    ...
    thor.confirm("Install " .. SERVER.label .. " via: " .. command .. "?", function()
        ...
        if not ok then
            thor.print("\n[basedpyright-setup] could not add a task (" .. err .. "); run this yourself instead:\n> " .. command .. "\n")
            return
        end
        thor.print("\n[basedpyright-setup] added task \"" .. task_name .. "\" — run it from the titlebar Tasks selector.\n" ..
...
thor.menu("LSP Setup", {
    {label = "basedpyright: Check Status",     command = "basedpyright-status"},
    {label = "basedpyright: Install Missing…", command = "basedpyright-install"},
})
