-- Detects whether clangd answers on PATH and, if not, adds a workspace task
-- that installs it; also detects the project's build system and helps
-- produce the compile_commands.json clangd needs (see buildsystem.lua).
-- Installers commonly run well past the two-second budget a plugin call
-- gets, and thor.exec kills everything left in the command's process tree
-- the moment the call that started it returns — so the install itself has
-- to run in the console (via a task), not through thor.exec directly.
...
local SERVER = {
    ...
    hint = "Also needs a compile_commands.json from your build — see \"Configure compile_commands.json…\" below.",
}
...
thor.on_command("clangd-status", function()
    local path = which(SERVER.bin)
    local out = "# " .. SERVER.label .. " Setup\n\nChecked against: `" .. OS_NAME .. "`\n\n" ..
        (path ~= "" and ("found: `" .. path .. "`\n") or "not found on PATH\n")
    thor.doc(".thor/clangd-status.md", out, true)
end)

thor.on_command("clangd-install", function()
    if which(SERVER.bin) ~= "" then
        thor.print("\n[clangd-setup] " .. SERVER.label .. " is already on PATH\n")
        return
    end

    local command = install_command()
    if not command then
        thor.print("\n[clangd-setup] " .. SERVER.label .. ": no automatic installer for " .. OS_NAME .. "\n" ..
    ...
    thor.confirm("Install " .. SERVER.label .. " via: " .. command .. "?", function()
        ...
        if not ok then
            thor.print("\n[clangd-setup] could not add a task (" .. err .. "); run this yourself instead:\n> " .. command .. "\n")
            return
        end
        thor.print("\n[clangd-setup] added task \"" .. task_name .. "\" — run it from the titlebar Tasks selector.\n" ..
...
thor.on_command("clangd-configure", function()
    buildsystem.configure({ which = which, add_task = add_task, os_name = OS_NAME })
end)

thor.menu("LSP Setup", {
    {label = "clangd: Check Status",                     command = "clangd-status"},
    {label = "clangd: Install Missing…",                 command = "clangd-install"},
    {label = "clangd: Configure compile_commands.json…", command = "clangd-configure"},
})
