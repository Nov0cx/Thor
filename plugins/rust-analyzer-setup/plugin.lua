-- Detects whether rust-analyzer answers on PATH and, if not, adds a
-- workspace task that installs it. Installers commonly run well past the
-- two-second budget a plugin call gets, and thor.exec kills everything left
-- in the command's process tree the moment the call that started it returns
-- — so the install itself has to run in the console (via a task), not
-- through thor.exec directly.
...
thor.on_command("rust-analyzer-status", function()
    local path = which(SERVER.bin)
    local out = "# " .. SERVER.label .. " Setup\n\nChecked against: `" .. os_name() .. "`\n\n" ..
        (path ~= "" and ("found: `" .. path .. "`\n") or "not found on PATH\n")
    thor.doc(".thor/rust-analyzer-status.md", out, true)
end)

thor.on_command("rust-analyzer-install", function()
    if which(SERVER.bin) ~= "" then
        thor.print("\n[rust-analyzer-setup] " .. SERVER.label .. " is already on PATH\n")
        return
    end

    local command = install_command()
    if not command then
        thor.print("\n[rust-analyzer-setup] " .. SERVER.label .. ": no automatic installer for " .. os_name() .. "\n" ..
    ...
    thor.confirm("Install " .. SERVER.label .. " via: " .. command .. "?", function()
        ...
        if not ok then
            thor.print("\n[rust-analyzer-setup] could not add a task (" .. err .. "); run this yourself instead:\n> " .. command .. "\n")
            return
        end
        thor.print("\n[rust-analyzer-setup] added task \"" .. task_name .. "\" — run it from the titlebar Tasks selector.\n" ..
...
thor.menu("LSP Setup", {
    {label = "rust-analyzer: Check Status",     command = "rust-analyzer-status"},
    {label = "rust-analyzer: Install Missing…", command = "rust-analyzer-install"},
})
