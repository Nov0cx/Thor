...
// Loads the real plugins/compile-commands/plugin.lua against a fake host and
// drives its build-system detection. Whether clangd itself is installed is not
// tested here: Settings > Language Servers looks on PATH natively (see
// lang/lsp/executable.odin), so no plugin shells out for it any more.

// A workspace that already has compile_commands.json, or a .clangd that
// already names a database, is left alone entirely.
@(test)
test_compile_commands_already_present_is_noop :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
...
@(test)
test_compile_commands_cmake_adds_task_and_dot_clangd :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
...
@(test)
test_compile_commands_cmake_leaves_existing_dot_clangd_alone :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
...
@(test)
test_compile_commands_bazel_writes_snippet_and_task :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
...
// Re-running the command once hedron is already wired in must not
// prompt again or duplicate either file's content.
@(test)
test_compile_commands_bazel_is_idempotent :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
    ...
    manager_set_host(&m, fake_host(&env))
    manager_load(&m)
    testing.expect(t, manager_run_command(&m, "configure-compile-commands"), "first configure-compile-commands ran")
    manager_dialog_confirm(&m)
    ...
    testing.expect(t, manager_run_command(&m, "configure-compile-commands"), "second configure-compile-commands ran")
...
@(test)
test_compile_commands_bazel_legacy_workspace_only_hints :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
...
@(test)
test_compile_commands_make_posix :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
    ...
    testing.expect(t, manager_run_command(&m, "configure-compile-commands"), "configure-compile-commands ran without bear")
    manager_dialog_confirm(&m)
    ...
    testing.expect(t, manager_run_command(&m, "configure-compile-commands"), "configure-compile-commands ran with bear")
    tasks_json = env.files[resolved(".thor/tasks.json")]
...
@(test)
test_compile_commands_make_windows :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
    ...
    testing.expect(t, manager_run_command(&m, "configure-compile-commands"), "configure-compile-commands ran without compiledb")
    manager_dialog_confirm(&m)
    tasks_json := env.files[resolved(".thor/tasks.json")]
    ...
    testing.expect(t, manager_run_command(&m, "configure-compile-commands"), "configure-compile-commands ran with compiledb")
    tasks_json = env.files[resolved(".thor/tasks.json")]
...
@(test)
test_compile_commands_bare_script_posix_hint_only :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
...
@(test)
test_compile_commands_bare_script_windows_hint_only :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
...
@(test)
test_compile_commands_nothing_found :: proc(t: ^testing.T) {
    m: Manager
    manager_init(&m)
    defer manager_destroy(&m)
