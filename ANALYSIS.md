...
**Infrastructure** — all five fixed, see the commits between `shell: kill the process group on a
timeout` and `windows: raise an open window on posix`.
- ~~POSIX timed `shell.run` kills only `/bin/sh`, not the process group.~~ `run_status` now forks
  itself, puts the shell in its own group (`setpgid`) and ends it with `killpg`. The forked child
  also restores `SIGPIPE` to `SIG_DFL`, which a terminal or language server had left ignored
  process-wide.
- ~~A failed `inotify_add_watch` silently unwatches a subtree.~~ `ENOSPC`/`EMFILE`/`ENOMEM` now hand
  the whole tree to `poll_worker` mid-session, after a root `.Modified` resync hint. `scan_skip_dir`
  is applied to the inotify walk, so `node_modules` no longer consumes the cap.
- ~~`parse_keybind` has no `cmd`/`super` token.~~ The three modifier bools are now
  `input.Modifiers`, a bit_set in a new `input/` leaf package shared by `ui` and `setting`.
- ~~VsDevCmd is hardcoded `-arch=amd64`.~~ `msvc.native_arch` reads the machine at run time
  (`PROCESSOR_ARCHITEW6432`, so an emulated x64 process still sees ARM64) and `msvc.vsdevcmd_call`
  builds the flags once. `build.odin` targets `ODIN_ARCH` instead, since it names what is built.
- ~~`thor_focus_window` is a POSIX no-op.~~ It shells out to `xdotool`/`wmctrl`/`osascript` and
  returns whether the raise went; the status message names the failure instead of implying a raise.

Still open here: no ARM64 Windows CI job, so only the x64 VsDevCmd command line is asserted.
