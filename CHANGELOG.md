...
- Language servers answer for every language other than Odin: hover, go to definition, find all references, document symbols, signature help, completion and semantic colors. `settings/lsp.json` names the servers, and a workspace `.thor/lsp.json` overlays it by server `id`.
