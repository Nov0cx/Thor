...
// An empty Preload bakes every family, the pre-selective-startup pipeline.
@(private = "file")
check_full_async_load :: proc(t: ^testing.T) {
    begin_async_load("assets/fonts/fonts.json", "assets/icons/icons.json")
    finish_async_load()
    defer shutdown()
    ...
    // measure must walk the same shaped placement draw draws, or a
    ...
    for name in ([2]string {"JetBrainsMono", "Iosevka"}) {
        ...
        for line in ([?]string {"->", "=>", "!=", "::", "...", "```", "a -> b", "<==>", "|||>"}) {
            testing.expect_value(t, measure(line, 17, name), cell * cast(i32) len(line))
        }
        // Multiple lines measure as the widest line.
        testing.expect_value(t, measure("ab\nabcd\na", 17, name), cell * 4)

        // A tab advances to the next stop, not by one space. A tab already on a
        // stop advances a whole span, so it is never zero-width.
        testing.expect_value(t, measure("\t", 17, name), cell * 4)
        testing.expect_value(t, measure("a\t", 17, name), cell * 4)
        testing.expect_value(t, measure("abc\t", 17, name), cell * 4)
        testing.expect_value(t, measure("abcd\t", 17, name), cell * 8)
        testing.expect_value(t, measure("\tab", 17, name), cell * 6)
        ...
        testing.expect_value(t, measure("\t", 17, name, cell * 2), cell * 2)
        // Measuring a line in pieces totals the same as measuring it whole.
        piece := measure("ab", 17, name)
        testing.expect_value(t, piece + measure("\tc", 17, name, piece), measure("ab\tc", 17, name))
    }
    ...
    {
        cell := measure("0", 17, "JetBrainsMono")
        before := shape_cache_len(&shape_cache)
        testing.expect_value(t, measure("\ta", 17, "JetBrainsMono"), cell * 5)
        set_tab_width(8)
        testing.expect_value(t, tab_width(), 8)
        testing.expect_value(t, measure("\ta", 17, "JetBrainsMono"), cell * 9)
    ...
    if family, ok := families["JetBrainsMono"]; ok {
        ...
        set_ligatures(false)
        testing.expect(t, !ligatures_enabled(), "ligature toggle not recorded")
        ...
        cell := measure("0", 17, "JetBrainsMono")
        testing.expect_value(t, measure("->", 17, "JetBrainsMono"), cell * 2)
    ...
    w1 := measure(line, 17, "JetBrainsMono")
    w2 := measure(line, 17, "JetBrainsMono")
    testing.expect_value(t, w1, w2)

    before := shape_cache_len(&shape_cache)
    _ = measure("first unique line", 17, "JetBrainsMono")
    ...
    // family is monospace, which makes every expected width a cell count.
    {
        ...
        each := wrap("aaa bbb ccc", cell * 3, 17, "JetBrainsMono")
        ...
        long := wrap("aaaaaaaaaa", cell * 3, 17, "JetBrainsMono")
        ...
        one := wrap("aaa bbb", cell * 100, 17, "JetBrainsMono")
...
check_selective_preload :: proc(t: ^testing.T) {
    ...
    begin_async_load("assets/fonts/fonts.json", "assets/icons/icons.json", Preload {
    ...
    testing.expect_value(t, len(family_names()), 10)
    ...
    spawned := prebake_async(prebake_names[:], extra_sizes[:])
    testing.expect_value(t, spawned, 5) // 4 manifest sizes + 28
    published := 0
    for _ in 0 ..< 10_000 {
        published += pump_async()
    ...
    testing.expect_value(t, prebake_async(prebake_names[:], extra_sizes[:]), 0)
    testing.expect_value(t, pump_async(), 0)
}
