...
- `capabilities` — advertise **only what Thor can consume**, so servers do not
  ...
  - `textDocument.completion.completionItem: {snippetSupport: true,
    preselectSupport: true, documentationFormat: ["plaintext"],
    resolveSupport: {properties: ["detail", "documentation",
    "additionalTextEdits", "command"]}}` — the editor expands a template into a
    tabstop session (`widgets/snippet.odin`) and resolves the picked candidate
    before applying what it defers. `insertReplaceSupport`, `labelDetailsSupport`
    and `completionList.itemDefaults` stay out: without them a server must send
    one plain `TextEdit` per candidate and no shared defaults, which is the only
    shape the decoder reads.
...
| `Completion` | `textDocument/completion` | `completionProvider` | `CompletionList \| CompletionItem[]` → `lang.Completion_Item`. `label` is the row, `insert` the text (`textEdit.newText`, else `insertText`, else the label), `start`/`end` the byte range `textEdit` names (−1 when it names none, leaving the editor to replace the typed word), plus `filterText`, `sortText`, `preselect`, `additionalTextEdits`, `command`/`arguments` as raw JSON, `snippet` from `insertTextFormat`, and the item verbatim in `raw` for resolve. Sorted by `sortText` before it answers: a server returns candidates unordered and states the order it wants there. `isIncomplete` reaches `Result.incomplete`, which decides whether the popup narrows the list itself or asks again. Still dropped: `labelDetails` and `itemDefaults`, neither advertised. |
| `Resolve_Completion` | `completionItem/resolve` | `completionProvider.resolveProvider` | Sent only when the user picks a candidate, with that candidate's own JSON (`Request.item`) as the whole params. Only `additionalTextEdits` and `command` are read back — the text has already landed by then. TypeScript, Go and Rust servers compute an auto-import edit here and nowhere else, which is why this exists; a reply that filled nothing in answers `ok = false`. |
...
| `Code_Actions` | `textDocument/codeAction`, plus `codeAction/resolve` | `codeActionProvider` | Thor computes edits up front by design (`lang.odin:115-118`). Any returned `CodeAction` with no `edit` gets `codeAction/resolve` called **eagerly, on the same worker** (we may block); an item that still has no `edit` — a pure `command` — is **dropped**, since Thor has no `workspace/executeCommand` path. Same `old_text` reconstruction as Rename. The request's `range` is `[offset, end)` (`Request.end`, defaulting to `offset`), so a real selection reaches a selection-scoped action ("extract function"). `context.diagnostics` is not left empty: each `Document` caches its last publish/pull diagnostics verbatim (`Cached_Diagnostic`, `lang/lsp/document.odin`), and `server_code_action_diagnostics` (`server.odin`) replays the ones overlapping the request range back into the params — what lets a server that gates a fix on seeing its own diagnostic (basedpyright's "add missing import") actually offer it. |
...
| `Format` | `textDocument/formatting` | `documentFormattingProvider` | `TextEdit[] \| null`. A null or empty reply is "nothing to change" and still `ok = true` — `thor_apply_format` reads `!ok` as "fix syntax errors first", and a server has no other way to say "already clean". Every reply, however many edits it names, is spliced into a copy of `req.source` and **re-diffed** (`lang.diff_spans`, shared with the Odin printer's own `formatter.odin`) rather than handed to the applier as-is: most servers (gofmt, rustfmt) answer one edit replacing the whole file, and applying that verbatim would drag every cursor in the buffer to wherever it ends. `options.tabSize` comes from `Request.tab_size` (the buffer's `tab_width`, falling back to 4); `insertSpaces` is always `true` — Thor's editor has no real-tabs mode to advertise. |
| `Format_Range` | `textDocument/rangeFormatting` | `documentRangeFormattingProvider` | Same decoder as `Format`. `range` is `[offset, end)`, which the host aligns to whole lines before dispatch (a half-line range makes most formatters produce nonsense) — unlike `Code_Actions`' selection, which is sent exactly as the caret left it. |
| `Format_On_Type` | `textDocument/onTypeFormatting` | `documentOnTypeFormattingProvider` | Same decoder again, plus `ch: Request.trigger`, the character just typed. Only ever sent for a character in the server's own advertised `firstTriggerCharacter` / `moreTriggerCharacter` set (`Capabilities.on_type_triggers`, decoded once at `.Ready`) — `lang.manager_on_type_trigger` is the pre-dispatch check, so a host never fires this per keystroke speculatively. Debounced like completion; applied silently on every outcome, since an edit landing mid-typing is not something the user asked to see a status for, and a stale result (the buffer moved past the snapshotted revision) is refused by the ordinary edit-apply check. |

**LSP methods with no Thor kind** — named, not implemented:
`textDocument/typeDefinition`, `implementation`, `declaration`,
`documentHighlight`, `foldingRange` (Thor derives folds from tree-sitter for
*every* grammar, `ROADMAP.md:763-769`, so an LSP provider would be a
regression in reach), `inlayHint`, `codeLens`, `callHierarchy`,
`typeHierarchy`, `selectionRange`, `documentLink`, `linkedEditingRange`,
