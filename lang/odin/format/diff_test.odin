...
// ranges.
@(private)
apply_spans :: proc(old: string, spans: []Span) -> string {
	out: [dynamic]u8
	pos := 0
	for s in spans {
...
@(private)
assert_diff_reassembles :: proc(t: ^testing.T, old, new: string) {
...
test_diff_single_line_change_is_minimal :: proc(t: ^testing.T) {
	...
	assert_diff_reassembles(t, old, new)
}
