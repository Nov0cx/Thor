// Document and workspace symbol outlines, reference search, rename, and the
// symbol index's freshness across edits.
package odin
...
`
    ...
    req := lang.Request{kind = .References, path = "buffer.odin", ext = ".odin", source = src, offset = at}
...
test_references_index_incremental :: proc(t: ^testing.T) {
    ...
    refs :: proc(e: ^Engine, path, src, dir: string) -> lang.Result {
        req := lang.Request {
            kind      = .References,
            path      = path,
            ext       = ".odin",
